...
def _loadHeppyGlobalOptions(options):
    ...
    if options.optionFile:
        opt = json.load(open(options.optionFile, 'rb'))
...
def _processOneComponent(pp, comp, outdir, preprocessor, options):
    ...
    if isinstance(pp, str) or isinstance(pp, str):
        _loadHeppyGlobalOptions(options)
        cfo = SourceFileLoader(os.path.basename(pp).rstrip('.py'), pp).load_module()
        #cfo = imp.load_source(os.path.basename(pp).rstrip('.py'), pp, open(pp,'rb'))
...
if __name__ == "__main__":
    ...
    cfo = SourceFileLoader(os.path.basename(cfg).rstrip('.py'), cfg).load_module()
    #cfo = imp.load_source(os.path.basename(cfg).rstrip('.py'), cfg, open(cfg,'rb'))
    pp = cfo.POSTPROCESSOR
