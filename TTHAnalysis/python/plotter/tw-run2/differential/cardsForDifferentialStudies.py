...
def CardsCommand(prod, year, var, isAsimov, nthreads, outpath, region, noUnc, useFibre, extra):
    ...
    comm = commandscaff.format(outpath      = outpath_,
                               ...
                               mcafile   = mcafile_,
                               cutsfile  = cutsfile_,
                               uncs      = "" if region == "particle" else "--unc tw-run2/uncs-tw.txt --amc" if not noUnc and "Response" in region else "--unc tw-run2/uncs-tw_1j1tdiff_lep1pt.txt --amc" if not noUnc else "--amc",
