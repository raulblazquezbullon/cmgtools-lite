...
#        f5 = r.TFile.Open(pathtothings + "/CovMat_particlefidbin.root", "read")
        f5 = r.TFile.Open(pathtothings + "/CovMat_particlefidbin_v2.root", "read")
    else:
    ...
    if ty == "particle":
        ...
        haMC_dr      = deepcopy(f1.Get("tru_aMC_dr").     Clone("haMC_dr"))
        haMC_dr2     = deepcopy(f1.Get("tru_aMC_dr2").    Clone("haMC_dr2"))
        haMC_ds      = deepcopy(f1.Get("tru_aMC_ds").     Clone("haMC_ds"))
        haMC_ds_runn = deepcopy(f1.Get("tru_aMC_ds_runn").Clone("haMC_ds_runn"))
        hDataCov     = deepcopy(f1.Get("CovMat").         Clone("hDataCov"))
        f1.Close()
    elif ty == "particlefidbin":
        hDR          = deepcopy(f1.Get("tru").            Clone("hDR"))
        hDS          = deepcopy(f1.Get("tru_DS").         Clone("hDS"))
        ...
        hDataCov     = deepcopy(f5.Get("CovMat_fidbin").Clone("hDataCov"))
    ...
    plot.addHisto(hDR,          'hist',      'tW PH+P8 DR',       'L', 'mc')
    plot.addHisto(hDS,          'hist,same', 'tW PH+P8 DS',       'L', 'mc')
    plot.addHisto(hHerwig,      'hist,same', 'tW PH+H7 DR',       'L', 'mc')
    plot.addHisto(haMC_dr,      'hist,same', 'tW aMC+P8 DR',      'L', 'mc')
    plot.addHisto(haMC_dr2,     'hist,same', 'tW aMC+P8 DR2',     'L', 'mc')
    plot.addHisto(haMC_ds,      'hist,same', 'tW aMC+P8 DS',      'L', 'mc')
    plot.addHisto(haMC_ds_runn, 'hist,same', 'tW aMC+P8 DS dyn.', 'L', 'mc')
