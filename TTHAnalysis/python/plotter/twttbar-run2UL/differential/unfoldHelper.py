...
class DataContainer:
    ...
    def readAndStore(self):
        ...
        thelumi = vl.TotalLumi if self.year == "run2" else vl.LumiDict[self.year]
        scalevalone = thelumi
        if self.year == "run2":
...
#        plot.addHisto(unregularized,'hist,same','UnRegularized','L')
    ...
    def doUnfoldingForAllNuis(self):
        ...
        tru                        = vl.giveMeOneComparison(tmptfile, "bb4l",                       scaleval, self.var, part = True)
        ...
        plot2.displayedLumi   = vl.TotalLumi if self.year == "run2" else vl.LumiDict[self.year]
    ...
    def doBottomLineTest(self):
        ...
        tmptfile = r.TFile.Open(self.folderpath + '/particle.root')
        tru = deepcopy(tmptfile.Get('x_bb4l').Clone('tru'))
        tmptfile.Close()
