...
BCORE=" --s2v --tree ${TREE} ttW-multilepton/lepton-fr/lepton-mca-frstudies.txt object-studies/lepton-perlep.txt"
BCORE="${BCORE} -L ttW-multilepton/functionsTTH.cc"
if [[ "$TREE" == "treeProducerSusyMultilepton" ]]; then
    BCORE="${BCORE} --mcc ttW-multilepton/validation/mcc-cmg_as_nanoaod.txt"
...
for WP in $WPs; do
	...
	case $WP in
	    ...
	    *ptJ95*)    ptJI="ptJI95";;
        ...
        B0="$BASE -P $T ttW-multilepton/lepton-fr/make_fake_rates_sels.txt ttW-multilepton/lepton-fr/make_fake_rates_xvars.txt --groupBy cut --sP ${Num} " 
        #B0="$B0 --legend=TR --showRatio --ratioRange 0.41 1.59   --yrange 0 0.20 " 
        B0="$B0 --legend=TR --showRatio --ratioRange 0.00 1.99   --yrange 0 0.35 " 
	B1="${PLOTTER} -P $T ttW-multilepton/lepton-fr/make_fake_rates_plots.txt"
