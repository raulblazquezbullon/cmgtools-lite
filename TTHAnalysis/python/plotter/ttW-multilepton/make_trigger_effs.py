...
BCORE=" --s2v --tree NanoAOD ttW-multilepton/mca-2lss-mcdata-ttbar-trigger.txt "
BCORE="{BCORE} -L ttW-multilepton/functionsTTH.cc".format(BCORE=BCORE)

BASE="python mcEfficiencies.py {BCORE} --ytitle 'Trigger efficiency' ttW-multilepton/trigger-eff/cuts_trigger_eff.txt  ".format(BCORE=BCORE)
B0="{BASE} -P {ORIGIN}/NanoTrees_TTH_091019_v6pre/{YEAR}/ -P {ORIGIN}/NanoTrees_TTH_090120_v6_triggerFix/{YEAR} ttW-multilepton/trigger-eff/trigger_cuts.txt ttW-multilepton/2lss_3l_plots.txt --groupBy cut --year {YEAR} --Fs {{P}}/1_recl --Fs {{P}}/1_extraTriggersMET --mcc ttW-multilepton/lepchoice-ttH-FO.txt --split-factor=-1 --WA prescaleFromSkim -f -j {nCores}  ".format(BASE=BASE,YEAR=YEAR,ORIGIN=ORIGIN,nCores=nCores)
