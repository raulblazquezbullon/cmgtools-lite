...
case $ANALYSIS in
ttH) CUTFILE="ttW-multilepton/lepton-fr/fr-z3l.txt"; XVAR="l3CPt"; NUM="mva090";;
...
BCORE=" --s2v --tree treeProducerSusyMultilepton ttW-multilepton/lepton-fr/mca-fr-z3l.txt ${CUTFILE} -P $T -l  36.5 --AP --mcc ttW-multilepton/mcc-eleIdEmu2.txt "
BCORE="$BCORE --mcc ttW-multilepton/ttH_2lss3l_triggerdefs.txt -X ^Trig -A Z1 ptcuts 'LepGood_pt[0]>25 && LepGood_pt[1]>15' " # no trigger selection

lepton=$1; if [[ "$1" == "" ]]; then exit 1; fi
case $lepton in
mu) BCORE="${BCORE} -E ^${lepton} -E tightZ1 ";; #;--mcc ttW-multilepton/mcc-ichepMediumMuonId.txt "; ;;
el) BCORE="${BCORE} -E ^${lepton} -E tight70";; #--mcc ttW-multilepton/mcc-ichepMediumMuonId-fake.txt"; ;;
...
PUW=" -L ttW-multilepton/lepton-fr/frPuReweight.cc -W 'puwZ3l(nVert)' "

case $what in
    nvtx)
        echo "python mcPlots.py -f -j 6 $BCORE ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE --sP nvtx $EWKONE " 
        echo "echo; echo; ";
        ...
        ;;
    nvtx-closure)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE --sP nvtx $EWKONE  --showRatio --maxRatioRange 0.8 1.2 " 
        ;;
    fit*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE -E $what $FITEWK --preFitData ${what/fit/} --sP ${what/fit/} --showRatio --maxRatioRange 0.0 1.99 $* " 
        ;;
    num-fit*)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE -E $what $FITEWK --preFitData ${what/num-fit/} --sP ${what/num-fit/} --showRatio --maxRatioRange 0.0 1.99 -E num $* " 
        ;;
    prefit-num|prefit-den)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE -E ${what/prefit-/} --showRatio --maxRatioRange 0.0 1.99 --sP mZ1,mtW3,met_log,l3.* " 
        ;;
    scaled-num|scaled-den)
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE $FITDY -E ${what/scaled-/} --showRatio --maxRatioRange 0.0 1.99 --sP mZ1,mtW3,met_log,l3.* --preFitData mtW3 " 
        ;;
    cuts-num|cuts-den)
        CUTS="  -A 3l mt25 'met_pt < 25' "
        echo "python mcPlots.py -f -j 6 $BCORE $PUW ttW-multilepton/lepton-fr/fr-z3l_plots.txt --pdir $PBASE $FITDY -E ${what/cuts-/} --showRatio --maxRatioRange 0.0 1.99 --sP mtW3,met_log,l3CPt_c --preFitData mtW3 $CUTS " 
        ;;
    fakerates-old)
        MCEFF="  python mcEfficiencies.py -f  $BCORE $PUW $SCALEWK --groupBy cut ttW-multilepton/lepton-fr/fr-z3l_pass.txt  ttW-multilepton/lepton-fr/fr-z3l_plots.txt  "
    ...
    fakerates-*)
        ...
        MCEFF="  python ttW-multilepton/dataFakeRate.py -f  $BCORE $PUW -p DY,VZ,data --groupBy cut ttW-multilepton/lepton-fr/fr-z3l_pass.txt  ttW-multilepton/lepton-fr/fr-z3l_plots.txt  "
        MCEFF="$MCEFF --sp DY --sP ${NUM}  "
        ...
        STACK="python ttW-multilepton/lepton-fr/stack_fake_rates_data.py $RANGES $LEGEND "
