...
def binEdges(axis,nbins):
    r0 = [ axis.GetBinLowEdge(1) ] + [ axis.GetBinUpEdge(i) for i in range(1,nbins+1) ]
    return array('f',r0)
def yzslice(h,ix):
...
for ispec in args[:-1]:
    ...
    ptMin,ptMax = list(map(float,conePtRange.split("-")))
    ...
    for ix in range(1,nx+1): 
        ...
        if (ptMin <= xmin and xmax <= ptMax):
            ptbins[(xmin,xmax)].append( dict((p,yzslice(h,ix)) for (p,h) in allh.items()) )
    if yedges == None:
        yedges = binEdges(hdata.GetYaxis(), hdata.GetNbinsY())
        zedges = binEdges(hdata.GetZaxis(), hdata.GetNbinsZ())
    tfin.Close()
allbins = list(sorted(ptbins.keys()))
