...
#dowhat = "dumps" 
#dowhat = "yields" 
#dowhat = "ntuple" # syntax: python ttW-multilepton/ttH_plots.py no 2lss_SR_extr outfile_{cname}.root --sP var1,var2,...
dojeccomps=True
...
def base(selection):
    ...
    CORE+=" -f -j %d -l %s -L ttW-multilepton/functionsTTH.cc --tree NanoAOD --mcc ttW-multilepton/lepchoice-ttH-FO.txt --split-factor=-1 --WA prescaleFromSkim --year %s  --mcc ttW-multilepton/mcc-METFixEE2017.txt"%(nCores, lumis[YEAR],YEAR if YEAR!='all' else '2016,2017,2018')# --neg" --s2v 
    RATIO= " --maxRatioRange 0.0  1.99 --ratioYNDiv 505 "
    RATIO2=" --showRatio --attachRatioPanel --fixRatioRange "
    LEGEND=" --legendColumns 3 --legendWidth 0.6 "
    ...
    if selection=='2lss':
        ...
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*bTagWeight*leptonSF_2lss*triggerSF_2lss'"%GO
        ...
        GO="%s ttW-multilepton/mca-3l-mc.txt ttW-multilepton/3l_tight_legacy.txt "%CORE
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*bTagWeight*leptonSF_3l*triggerSF_3l'"%GO
        ...
        GO="%s ttW-multilepton/mca-4l-mc.txt ttW-multilepton/4l_tight.txt "%CORE
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*bTagWeight*leptonSF_4l*triggerSF_3l'"%GO
        if dowhat in ["plots","ntuple"]: GO+=" ttW-multilepton/2lss_3l_plots.txt --xP '^(2|3)lep_.*' --xP '^lep(1|2|3|4)_.*' --xP 'kinMVA_.*' "
...
def runIt(GO,name,plots=[],noplots=[]):
    ...
    if dowhat == "plots":  
        if not ('forcePlotChoice' in sys.argv[4:]): print(submit.format(command=' '.join(['python3 mcPlots.py',"--pdir %s/%s/%s"%(ODIR,YEAR,name),GO,' '.join(['--sP %r'%p for p in plots]),' '.join(['--xP %r'%p for p in noplots]),' '.join(sys.argv[4:])])))
        else: print('python3 mcPlots.py',"--pdir %s/%s/%s"%(ODIR,YEAR,name),GO,' '.join([x for x in sys.argv[4:] if x!='forcePlotChoice']))
    elif dowhat == "yields": print('echo %s; python3 mcAnalysis.py'%name,GO,' '.join(sys.argv[4:]))
    elif dowhat == "dumps":  print('echo %s; python3 mcDump.py'%name,GO,' '.join(sys.argv[4:]))
    elif dowhat == "ntuple": print('echo %s; python3 mcNtuple.py'%name,GO,' '.join(sys.argv[4:]))
def add(GO,opt):
    return '%s %s'%(GO,opt)
def setwide(x):
    x2 = add(x,'--wide')
...
if __name__ == '__main__':
    ...
    if '2lss_' in torun:
        ...
        if '_unc' in torun:
            x = add(x,"--unc ttW-multilepton/systsUnc.txt  --xu CMS_ttHl_TTZ_lnU,CMS_ttHl_TTW_lnU")
    ...
    if 'cr_3l' in torun:
        x = base('3l')
        x = add(x,"-I 'Zveto' -E ^underflowVeto3l ")
        if '_data' in torun: 
            x = x.replace('mca-3l-mc.txt','mca-3l-mcdata.txt')
        if '_frdata' in torun:
            x = promptsub(x)
        ...
        if 'bybin' in torun:
            ...
            plots = ['lep1_conePt','lep2_conePt','lep3_conePt', 'lep1_eta', 'lep2_eta', 'lep3_eta', 'jet1_pt','jet2_pt','3lep_m3l']
            x = add(x,"-A tauveto bin1 'ttH_3l_clasifier(nJet25, nBJetMedium25)==1' ")
           
            x = add(x,'--pdir %s/%s/cr_3l_data_frdata_unc_bybin/bin1'%(ODIR,YEAR))
            for i in range(1,13) :
               
               if i==1:
                  runIt(x,'%s'%torun,plots)
               else:
                  print(("--pdir %s/%s/cr_3l_data_frdata_unc_bybin"%(ODIR,YEAR)))
                  x = x.replace('--pdir %s/%s/cr_3l_data_frdata_unc_bybin/bin%s'%(ODIR,YEAR,str(i-1)),'--pdir %s/%s/cr_3l_data_frdata_unc_bybin/bin%s'%(ODIR,YEAR,str(i)))
                  x = x.replace("-A tauveto bin"+str(i-1)+" 'ttH_3l_clasifier(nJet25, nBJetMedium25)=="+str(i-1)+"'","-A tauveto bin"+str(i)+" 'ttH_3l_clasifier(nJet25, nBJetMedium25)=="+str(i)+"'")
                  runIt(x,'%s'%torun,plots)
        else:
            runIt(x,'%s'%torun,plots)
    if 'cr_4l' in torun:
        x = base('4l')
        x = add(x,"-I ^Zveto  -E ^underflowVeto4l ")
        if '_data' in torun: 
            x = x.replace('mca-4l-mc.txt','mca-4l-mcdata.txt')
        if '_frdata' in torun:
            x = promptsub(x)
