...
cd /nfs/fanae/user/rblazquez/TFG/wz_run3/CMSSW_12_4_12/src/CMGTools/TTHAnalysis/python/plotter/check_discriminant_vars/rebin8/m3l/combined_cards
...
if [ ${SLURM_ARRAY_TASK_ID} -eq 7 ]; then
  combine -M MultiDimFit -n _paramFit_Test_prop_binwz-card_bin0_fakes_DY --algo impact --redefineSignalPOIs r_prompt_WZ -P prop_binwz-card_bin0_fakes_DY --floatOtherPOIs 1 --saveInactivePOI 1 -t -1 --X-rtd MINIMIZER_MaxCalls=99999999999 --maxFailedSteps 500 --X-rtd MINIMIZER_analytic --setRobustFitTolerance 0.05 --cminPreScan --cminDefaultMinimizerStrategy 0 -m 125 -d wz-ws.root --setParameters r_prompt_WZ=1,r_prompt_ZZ=1
fi
