...
TFile* f_likelihood = new TFile(DATA_WZ+"/WZ_MC/likelihood2016_function.root", "read");
...
RooWorkspace * w_mll = dynamic_cast<RooWorkspace*>( f_likelihood->Get("w_mll") );
RooWorkspace * w_met = dynamic_cast<RooWorkspace*>( f_likelihood->Get("w_met") );
RooWorkspace * w_mtW = dynamic_cast<RooWorkspace*>( f_likelihood->Get("w_mtW") );

float getMLLVal(float mll){
  w_mll->var("x")->setVal(mll);
  float out = w_mll->pdf("sum")->getVal(w_mll->set("observables"));
  return out; 
}

float getMETVal(float met){
  w_met->var("x")->setVal(met);
  float out = w_met->pdf("sum")->getVal(w_met->set("observables"));
  return out; 
}

float getMTWVal(float mtW){
  w_mtW->var("x")->setVal(mtW);
  float out = w_mtW->pdf("sum")->getVal(w_mtW->set("observables"));
  return out; 
}

float getLikelihoodFit(float met, float mtW, float mll){
  return getMLLVal(mll)*getMETVal(met)*getMTWVal(mtW);
}
