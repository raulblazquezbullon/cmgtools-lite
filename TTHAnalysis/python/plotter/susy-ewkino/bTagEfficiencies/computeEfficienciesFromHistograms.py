...
inFile = ROOT.TFile("/nfs/fanae/user/carlosec/www/EWKino/ewk_2017_btag//plots_beff.root", "READ")
    ...
    for wp in WPs:
        ...
        for f in flavors.keys():
            ...
            if doControlPlots:
                ...
                c.SaveAs("./plots/eff_TT_%s_%s_%s.pdf"%(wp,flavors[f], alg))
                c.SaveAs("./plots/eff_TT_%s_%s_%s.png"%(wp,flavors[f], alg))
                c.SaveAs("./plots/eff_TT_%s_%s_%s.root"%(wp,flavors[f], alg))
    theAlgFile.Close()
