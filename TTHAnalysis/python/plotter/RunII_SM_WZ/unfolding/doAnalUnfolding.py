        ...
        if not doSignSplit:
            for iShape, iPack in self.responsePairs.items():
                ...
                for iChan, iChanCut in self.chanCuts.items():
                    ...
                    print('sbatch -p infinite -c {nthreads} --wrap "python makeShapeCardsSusy.py RunII_SM_WZ/unfolding/mca_unfolding_{year}.txt RunII_SM_WZ/{year}/cuts_fiducial.txt \'{iShape}\' \'{iRange}\' RunII_SM_WZ/{year}/systs_wz.txt  --tree treeProducerSusyMultilepton -P /pool/cienciasrw/HeppyTrees/RA7/nanoAODv5_{year}_unSkimmed_estructure/   --Fs {{P}}/leptonPtCorrections/ {prefiringTrees} {pileupTrees} --Fs {{P}}/leptonJetReCleanerWZSM/ --Fs {{P}}/leptonBuilderWZSM/ {triggerTrees} --FMCs {{P}}/leptonMatcher/ --FMCs {{P}}/bTagEventWeights/ --FMCs {{P}}/lepgenVarsWZSM/ --Fs {{P}}/bosonPolarization/ -L RunII_SM_WZ/functionsSF.cc -L RunII_SM_WZ/functionsMCMatch.cc -L RunII_SM_WZ/functionsWZ.cc -W \'getLeptonSF_v4(0,0,{year},LepSel_pt[0],LepSel_eta[0],LepSel_pdgId[0])*getLeptonSF_v4(0,0,{year},LepSel_pt[1],LepSel_eta[1],LepSel_pdgId[1])*getLeptonSF_v4(0,0,{year},LepSel_pt[2],LepSel_eta[2],LepSel_pdgId[2])*{pileupWeights}*bTagWeightDeepCSVT_nom{prefiringWeights}\' --obj Events -j {nthreads} -l {lumi} -f  -E SRWZ {perchan} {chargecut} --neglist promptsub --asimov --autoMCStats  --XTitle \\"{iXTitle}\\" --YTitle \\"{iYTitle}\\" -o WZSR_{year}" '.format(year=self.year,prefiringTrees=prefiringTrees[self.year],prefiringWeights=prefiringWeights[self.year],triggerTrees=triggerTrees[self.year],lumi=lumi[self.year],pileupTrees=pileupTrees[self.year],pileupWeights=pileupWeights[self.year], perchan=perchan, nthreads=self.nthreads,iShape=iShape, iRange=iRange, iXTitle=iXTitle, iYTitle=iYTitle, chargecut=iChanCut) )
        ...
        for charge in [ '_none', '_plus', '_minus' ]:
            for fs in [ 'incl', 'eee', 'eem', 'mme', 'mmm']:
                if not base:
                    ...
                    ret.append('sbatch -p infinite -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -a -o {year}/unfold_nobias/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_nobias_data_{fs}.log"'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ...
                    ret.append('sbatch -p infinite -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -a -o {year}/unfold_1p0bias/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_1p0bias_data_{fs}.log"'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ...
                    ret.append('sbatch -p infinite -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -a -o {year}/unfold_nnlobias/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_nnlobias_data_{fs}.log"'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
