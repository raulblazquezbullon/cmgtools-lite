        ...
        for charge in [ '_none', '_plus', '_minus' ]:
            for fs in [ 'incl', 'eee', 'eem', 'mme', 'mmm']:
                if not base:
                    ...
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -o {year}/unfold_nobias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR_{year}.input.root -r --closure >& logs/{year}_nobias_{charge}_noconstraintarea_mcclosure_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
            
                    # Bias 1, area constraint
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -a -o {year}/unfold_1p0bias/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_1p0bias_{charge}_data_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -a -o {year}/unfold_1p0bias/{fs}{charge}/mcclosure -c common/WZSR_{year}.input.root -r --closure >& logs/{year}_1p0bias_{charge}_mcclosure_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
                
                    # Bias 1, no area constraint
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -o {year}/unfold_1p0bias_noconstraintarea/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_1p0bias_{charge}_noconstraintarea_data_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -o {year}/unfold_1p0bias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR_{year}.input.root -r --closure >& logs/{year}_1p0bias_{charge}_noconstraintarea_mcclosure_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
        
                    # Bias 1.13, no area constraint
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -o {year}/unfold_nnlobias_noconstraintarea/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_nnlobias_{charge}_noconstraintarea_data_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -o {year}/unfold_nnlobias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR_{year}.input.root -r --closure >& logs/{year}_nnlobias_{charge}_noconstraintarea_mcclosure_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))

                else:
                    # Bias 1.13, area constraint
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -a -o {year}/unfold_nnlobias/{fs}{charge}/data -c common/WZSR_{year}.input.root -r >& logs/{year}_nnlobias_{charge}_data_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
                    ret.append('sbatch -p {queue} -c {nthreads} --wrap "python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -a -o {year}/unfold_nnlobias/{fs}{charge}/mcclosure -c common/WZSR_{year}.input.root -r --closure >& logs/{year}_nnlobias_{charge}_mcclosure_{fs}.log"'.format(queue=self.queue,outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,nthreads=self.nthreads,charge=charge))
