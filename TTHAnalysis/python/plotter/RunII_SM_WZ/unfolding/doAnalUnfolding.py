        ...
        if not doSignSplit:
            ...
            for iWhat, iCut in enabler.items():
                ...
                for iShape, iPack in self.responsePairs.items():
                    ...
                    for iChan, iChanCut in self.chanCuts.items():
                        perchan = ' --od {outdir}/responses/{year}_{chan}_fitWZonly_{iout}_{chargeinfo}/ --bin {chan} -E SRWZ {cut} '.format(outdir=self.outputDir, year=self.year,chan=iChan, cut=iChanCut, iiout=iOut,chargeinfo=iWhat)
        ...
        for charge in [ '_none', '_plus', '_minus' ]:
            for fs in [ 'incl', 'eee', 'eem', 'mme', 'mmm']:
                if not base:
                    ...
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -a -o unfold_nobias/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/nobias_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -a -o unfold_nobias/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/nobias_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))

                    # No bias, no area constraint
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -o unfold_nobias_noconstraintarea/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/nobias_noconstraintarea_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 0 -o unfold_nobias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/nobias_noconstraintarea_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
            
                    # Bias 1, area constraint
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -a -o unfold_1p0bias/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/1p0bias_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -a -o unfold_1p0bias/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/1p0bias_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                
                    # Bias 1, no area constraint
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -o unfold_1p0bias_noconstraintarea/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/1p0bias_noconstraintarea_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.0 -o unfold_1p0bias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/1p0bias_noconstraintarea_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
        
                    # Bias 1.13, no area constraint
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -o unfold_nnlobias_noconstraintarea/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/nnlobias_noconstraintarea_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -o unfold_nnlobias_noconstraintarea/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/nnlobias_noconstraintarea_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))

                else:
                    # Bias 1.13, area constraint
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -a -o unfold_nnlobias/{fs}{charge}/data -c common/WZSR.input.root -r >& logs/nnlobias_data_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
                    ret.append('python {executable} -i {outputdir} -y {year} -f {fs} --charge {charge}  -b 1.13 -a -o unfold_nnlobias/{fs}{charge}/mcclosure -c common/WZSR.input.root -r --closure >& logs/nnlobias_mcclosure_{fs}.log'.format(outputdir=self.outputDir,year=self.year,executable=executable,fs=fs,charge=charge))
