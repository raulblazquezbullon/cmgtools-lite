...
#            ROOT.gEnv.SetValue("XNet.Debug", -1); # suppress output about opening connections
        ...
        friendOpts += [ ('sf/t', d+"/evVarFriend_{cname}.root") for d in (self._options.friendTreesFastSimSimple if self._isfastsim else self._options.friendTreesFullSimSimple)]
...
def addTreeToYieldOptions(parser):
    ...
    parser.add_option("--FFulls", "--add-friend-full-simple",    dest="friendTreesFullSimSimple",  action="append", default=[], nargs=1, help="Add friends in a directory to FullSim only. The rootfile must be called evVarFriend_{cname}.root and tree must be called 't' in a subdir 'sf' inside the rootfile.") 
    parser.add_option("--FFasts", "--add-friend-fast-simple",    dest="friendTreesFastSimSimple",  action="append", default=[], nargs=1, help="Add friends in a directory to FastSim only. The rootfile must be called evVarFriend_{cname}.root and tree must be called 't' in a subdir 'sf' inside the rootfile.") 
