...
outDatasets = "2016_dataobs_bootstrap_quantiles.root"
  ...
  for p in processes:
    ...
    if ref==False:
      ...
      for i in range(1, tempH[p].GetNbinsX()+1):
        ...
        if newN <= 0 or meanN <= 0:
          tempH[p].SetBinContent(i, 0)
        else:
          tempH[p].SetBinContent(i, newW)

        #print meanN, newN, meanW, newW, weiH[p], p, tempH[p].GetBinContent(i)
