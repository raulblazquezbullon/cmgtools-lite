...
import copy
import array
from scipy.stats import poisson as ppvar
...
central  = "/nfs/fanae/user/carlosec/CMSSW_10_4_0/src/CMGTools/TTHAnalysis/python/plotter/RunII_SUSY_EWK/utils/../../LEE_toys_set1_dM[M]/cuts_3l.input.root"
varsTemp = "/nfs/fanae/user/carlosec/CMSSW_10_4_0/src/CMGTools/TTHAnalysis/python/plotter/RunII_SUSY_EWK/utils/../../LEE_toys_[TAG]_dM[M]/cuts_3l.input.root"

tags = {"set1":[1,100], "set2":[101,200], "set3":[201,300], "set4":[301,400], "set5":[401,500], "set6":[501,750], "set7":[751,999]}


signalInjections = []#"x_sig_TChiWZ_200_110","x_sig_TChiWZ_350_1","x_sig_TChiWZ_125_85"]

print signalInjections
outDatasets = "2016_dataobs_bootstrap_naive.root"
...
def extract_histos(fil, m, ref=False, w=0, addHisto=False):
  ...
  for p in processes:
    ...
    if ref==False:
      ...
      #  continue
      #meanN = centH[p].GetBinContent(i)/weiH[p]
      #newN  = tempH[p].GetBinContent(i)/weiH[p]
      #meanW = centH[p].GetBinContent(i)
      #theC = ppvar.cdf(newN,meanN)
      #newW = ppvar.ppf(theC, meanW)
      #tempH[p].SetBinContent(i, newW)
  ...
  if ref==False:
    ...
    if toAdd:
      ...
      extra = ""
    data_obs = centH["prompt_WZ"].Clone("data_obs"+extra+ str(w) + "_dM"+m + extra)
    ...
    for i in range(1,data_obs.GetNbinsX()+1):
      data_obs.SetBinContent(i, ppvar.rvs(data_obs.GetBinContent(i)))
      data_obs.SetBinError(i, data_obs.GetBinContent(i)**0.5)
      #print data_obs.GetBinContent(i)
    outFile.cd()
    data_obs.Write()
