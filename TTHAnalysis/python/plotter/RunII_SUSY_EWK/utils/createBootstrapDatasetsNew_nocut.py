...
outDatasets = "2016_dataobs_bootstrap_normnocut.root"
...
def extract_histos(fil, m, ref=False, w=0, addHisto=False):
  ...
  if ref==False:
    ...
    for i in range(1,data_obs.GetNbinsX()+1):
      data_obs.SetBinContent(i, data_obs.GetBinContent(i))
    ...
    for i in range(1,data_obs.GetNbinsX()+1):
      data_obs.SetBinContent(i, data_obs.GetBinContent(i))
      data_obs.SetBinError(i, max(0,data_obs.GetBinContent(i))**0.5)
      #print data_obs.GetBinContent(i)
    outFile.cd()
    data_obs.Write()
