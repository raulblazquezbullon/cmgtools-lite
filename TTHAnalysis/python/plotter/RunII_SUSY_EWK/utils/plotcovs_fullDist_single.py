...
def produceVals(coms):
  ...
  for w in range(1,500):
    inC1 = inF.Get("data_obs_weight0_dM%i"%dM1)
    inC2 = inF.Get("data_obs_weight0_dM%i"%dM2)
  ...
  c.SaveAs("~/www/plots_bootstrap_newstrat/covs2D_v2/covs%s_%i_%i.pdf"%(typ,dM1,dM2))
  c.SaveAs("~/www/plots_bootstrap_newstrat/covs2D_v2/covs%s_%i_%i.png"%(typ,dM1,dM2))
...
commands = []
for m1 in masses:
    #for m2 in [100]:#masses:
    commands.append([inFNorm,m1,m1,"Norm"])
