...
float triggerSF_2016(float pt1  , float eta1  , int pdg1) {
    float lpt = pt1; float leta = eta1; int lpdg = pdg1;
    TH2F* hist = (abs(lpdg)==13)?h_trigSF_1mu:h_trigSF_1el;
    return getSF(hist, lpt, abs(leta));
}
...
TFile* f_photonSF_v4_2016  = new TFile(DATA_SF+"/photonSF/Fall17V2_2016_MVAwp90_photons.root", "read");
TH2F*  h_photonSF_v4_2016  = (TH2F*) f_photonSF_v4_2016->Get("EGamma_SF2D");


float getPhotonSF_v4_2016(int wp, int unc, float pt, float eta){
  float sfTtoL     = getSF(h_photonSF_v4_2016, eta, pt);
  return sfTtoL;
}

TFile* f_photonSF_v4_2017  = new TFile(DATA_SF+"/photonSF/2017_PhotonsMVAwp90.root", "read");
TH2F*  h_photonSF_v4_2017  = (TH2F*) f_photonSF_v4_2017->Get("EGamma_SF2D");


float getPhotonSF_v4_2017(int wp, int unc, float pt, float eta){
  float sfTtoL     = getSF(h_photonSF_v4_2017, eta, pt);
  return sfTtoL;
}

TFile* f_photonSF_v4_2018  = new TFile(DATA_SF+"/photonSF/2018_PhotonsMVAwp90.root", "read");
TH2F*  h_photonSF_v4_2018  = (TH2F*) f_photonSF_v4_2018->Get("EGamma_SF2D");


float getPhotonSF_v4_2018(int wp, int unc, float pt, float eta){
  float sfTtoL     = getSF(h_photonSF_v4_2018, eta, pt);
  return sfTtoL;
}
...
TFile* f_muSF_v4_2016_LtoT_mva       = new TFile(DATA_SF+"/leptonSF/lepMVAEffSF_m_3l_2016.root", "read");
...
float getMuSF_v4_2016(int wp, int unc, float pt, float eta){
  ...
  //And then loose leptonMVA to tight leptonMVA
  float sfLtoT     =  getSF(h_muSF_v4_2016_LtoT_mva, eta, pt);
  float sfLtoTcorr = getUnc(h_muSF_v4_2016_LtoT_mva, eta, pt);
