...
def _loose_electron(lep):
    ...
    if not (abs(lep.dz)<0.1): return False
    if not (lep.miniPFRelIso_all < 0.4): return False
    if not (lep.mvaNoIso_WPL): return False
    if (lep.lostHits): return False
    # -- OLD: in nanoAOD lostHits is a char_t, 
    # but it is converted to boolean when LepGood collection is created.
...
def _fO_electron(lep, btagWPM,jetlist):
    ...
    if not ((lep.mvaNoIso_WPL and lep.mvaTTH > 0.80) or lep.mvaNoIso_WP80): return False
    if (lep.lostHits): return False
    # -- OLD: in nanoAOD lostHits is a char_t, 
    # but it is converted to boolean when LepGood collection is created.
#    if (ord(lep.lostHits) > 1): return False
...
#    if not lep.Electron_cutBased: return False
    if not (lep.mvaTTH > 0.8): return False
    return True

def _tight_electron_runIII(lep, btagWPM,jetlist):
...
def _tight_lepton_runIII(lep, btagWPM, btagWPL,jetlist):
    return _tight_muon_runIII(lep,btagWPM,btagWPL,jetlist) if abs(lep.pdgId) == 13 else _tight_electron_runIII(lep,btagWPM,jetlist)
