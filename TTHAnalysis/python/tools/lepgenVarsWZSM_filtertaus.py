...
## lepgenVarsWZSM
## ___________________________________________________________________
class lepgenVarsWZSM_filtertaus:
    ...
    def collectObjects(self, event):
        ...
        self.genleps = filter(lambda x: not(x.hasTauAnc), self.genleps)
        self.genparts     = [l             for l  in Collection(event, "GenPart", "nGenPart")  ]
        self.genleps_notd = filter(lambda x: self.filterGenPart(x), self.genparts)
        #Need to add a filter so we don't take events with taus anyway (as this can't be obtained from GenDressedLepton in any capacity
        if any([abs(p.pdgId) == 15 for p in self.genleps_notd]): self.genleps = []
        self.genleps = filter(lambda x: self.match_notdgood(x, self.genleps_notd),  self.genleps)
    ...
    def match_notdgood(self, lep, coll):
       for p in coll:
          if deltaR(lep.eta, lep.phi, p.eta, p.phi) < 0.4 and lep.pdgId==p.pdgId:
              setattr(lep, "motherpdgId", p.motherpdgId)
              return True
       return False 

    def filterGenPart(self, p):
       #print p.pdgId, p.status, p.genPartIdxMother
       if not(abs(p.pdgId)==11 or abs(p.pdgId)==13): return False # or abs(p.pdgId)==15): return False
       #print "... passID"
       if not(p.status==23 or p.status==1): return False # or (p.status==2 and abs(p.pdgId)==15)): return False
       #print "... passStatus"
