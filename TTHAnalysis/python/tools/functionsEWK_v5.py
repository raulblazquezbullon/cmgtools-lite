...
def conept(lep):
    ...
    if not(lep.mvaTTH >= 0.8) and (abs(lep.pdgId) == 11): return 0.9*lep.pt* (1 + lep.jetRelIso)
    if not(lep.mvaTTH >= 0.85 and lep.mediumId) and (abs(lep.pdgId) == 13): return 0.75*lep.pt* (1 + lep.jetRelIso)
    else: return lep.pt
...
def _fO_muon(lep, btagWPM, btagWPL,jetlist):
    ...
    elif (lep.jetRelIso > 0.65): return False
