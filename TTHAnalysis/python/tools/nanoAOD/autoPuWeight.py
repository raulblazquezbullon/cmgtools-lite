...
class autoPuWeightModule( componentDependentModuleWrapper ):
    ...
    def initComponent(self, component):
        if component.isData:
            self._worker = None
        elif "Fall17" in component.dataset or '2017' in (component.dataset.split('/')[2]):
            self._worker = self._w2017()
        elif "Autumn18" in component.dataset or '2018' in (component.dataset.split('/')[2]):
            self._worker = self._w2018()
        elif "Summer16" in component.dataset or '2016' in (component.dataset.split('/')[2]):
