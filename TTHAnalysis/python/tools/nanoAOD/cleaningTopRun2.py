...
class cleaningTopRun2(Module):
    ...
    def beginJob(self, histFile = None, histDirName = None):
        ...
        self.branches.extend([var + realvar + self.label for var in self.outmasses for delta,realvar in self.systsLepEn.items()])
        ...
        for delta,sys in self.systsLepEn.items():
            ...
            print(getattr(self, "_helper_jetsSup"    + sys).cppImpl(), getattr(self, "_helper_jetsInf"    + sys).cppImpl(), getattr(self, "_helper_fwdjetsSup" + sys).cppImpl(), getattr(self, "_helper_fwdjetsInf" + sys).cppImpl(), end=' ')
    ...
    def initReaders(self, tree):
        ...
        for coll in [self.lc, self.jc] + [self.lc + var[1:] for delta,var in self.systsLepEn.items()]:
