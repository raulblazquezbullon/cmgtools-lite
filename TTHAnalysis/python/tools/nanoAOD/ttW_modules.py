...
modelspath = "/nfs/fanae/user/cvico/WorkSpace/wz-run3/release/CMSSW_12_4_12/src/CMGTools/TTHAnalysis/python/tools/nanoAOD/models/onnxConverter/"

modelspath1 = "/mnt_pool/c3_users/user/jriego/TFM/CMSSW_12_4_12/src/ml-ttw/saved_models/"

jecvars =  [ 'jes%s'%v for v in jecGroups] + ['jer%s'%x for x in ['barrel','endcap1','endcap2highpt','endcap2lowpt' ,'forwardhighpt','forwardlowpt']  ]  + ['HEM']
#mva_bdt_multiple = lambda : bdt_evaluator(modelspath + "BDT_ttw-multiple_50.pkl", jecvars=jecvars)
#mva_rf_multiple  = lambda : bdt_evaluator(modelspath + "RF_ttw-multiple_50.pkl", jecvars=jecvars)
mva_bdt_tt       = lambda : bdt_evaluator(modelspath + "BDT_ttw-ttbar_35.pkl", jecvars=jecvars)
#mva_rf_tt        = lambda : bdt_evaluator(modelspath + "RF_ttw-ttbar_35.onnx", jecvars=jecvars)

#mva_NN_tt        = lambda : bdt_evaluator(modelspath + "NN_ttW-ttbar_35.h5", jecvars=jecvars)
#mva_NN_multiple  = lambda : bdt_evaluator(modelspath + "NN_ttW-multiple_50.h5", jecvars=jecvars)

#mva_NN_tt_charge = lambda : bdt_evaluator(modelspath1 + "NN_ttW-ttbar.h5", jecvars=jecvars)

mvas_mc = [mva_bdt_tt]
#mvas_data = [mva_rf_tt_data]
