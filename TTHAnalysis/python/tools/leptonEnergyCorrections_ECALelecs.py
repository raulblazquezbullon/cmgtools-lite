...
class leptonEnergyCorrections_ECALelecs:
    ## __init__
    ...
    def __init__(self, inputlabel, muonFile, era):
        ...
        print os.path.join(os.path.dirname(__file__), 'RoccoR_cc.so')
        ...
        for l in leps:
          if not(isTau):
            ...
            if abs(l.pdgId) == 11:
                ...
                oldp4 = ROOT.TLorentzVector()
                oldp4.SetPtEtaPhiM(l.pt, l.eta, l.phi, l.mass)
                newp4 = ROOT.TLorentzVector() 
                newp4.SetPtEtaPhiE((oldp4.P()/(1+l.eInvMinusPInv*oldp4.P()))/math.cosh(l.eta+l.deltaEtaSC) , l.eta+l.deltaEtaSC , l.phi , oldp4.P()/(1+l.eInvMinusPInv*oldp4.P()))
                setattr(l,"pt", newp4.Pt())
                setattr(l,"eta", newp4.Eta())
                setattr(l,"phi", newp4.Phi())
                setattr(l,"mass", newp4.M())
    ...
    def listBranches(self):
        ...
        for var in ["pt","ptUp","ptDown","eta", "phi", "mass"]:
            biglist.append(("LepGood_corrected" + var+"_"+self.inputlabel, "F", 8))
            biglist.append(("TauGood_corrected" + var+"_"+self.inputlabel, "F", 8))
        return biglist
    ...
    def resetMemory(self, ev):
        ...
        #for i in range(8):
        for var in ["pt","ptUp","ptDown","eta", "phi", "mass"]:
                self.ret["LepGood_corrected" + var+"_"+self.inputlabel] = [-999.]*ev.nLepGood

        #for i in range(8):
        for var in ["pt","ptUp","ptDown","eta", "phi", "mass"]:
                self.ret["TauGood_corrected" + var+"_"+self.inputlabel] = [-999.]*ev.nTauGood
    ...
    def writeLepSel(self):
        for i, l in enumerate(self.lepCorr):
            for var in ["pt","ptUp","ptDown","eta", "phi", "mass"]:
                self.ret["LepGood_corrected" + var+"_"+self.inputlabel][i] = getattr(l, var, -999.)

        for i, l in enumerate(self.tauCorr):
            for var in ["pt","ptUp","ptDown","eta", "phi", "mass"]:
                self.ret["TauGood_corrected" + var+"_"+self.inputlabel][i] = getattr(l, var, -999.)
