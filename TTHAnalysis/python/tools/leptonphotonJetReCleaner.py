...
class LeptonPhotonJetReCleaner:

    def __init__(self,label,looseLeptonSel,cleaningLeptonSel,FOLeptonSel,tightLeptonSel,cleanJet,selectJet,cleanTau,looseTau,tightTau,cleanPhoton,loosePhoton,tightPhoton,cleanJetsWithTaus,cleanPhotonsWithLoose,cleanJetsWithPhotons,doVetoZ,doVetoLMf,doVetoLMt,jetPt,bJetPt,coneptdef,storeJetVariables=False,cleanTausWithLoose=False, year=2016, bAlgo="DeepCSV"):
        ...
        self.cleanPhoton = cleanPhoton
        self.loosePhoton = loosePhoton
        self.tightPhoton = tightPhoton
        self.cleanJetsWithPhotons = cleanJetsWithPhotons
        self.cleanPhotonsWithLoose = cleanPhotonsWithLoose
    ...
    def listBranches(self):
        ...
        biglist.extend([
                ("nPhotonSel"     +label, "I"), 
                ("nTightPhotonSel"+label, "I"), 
                ("iPhotonSel"+label,"I",20,"nPhotonSel"+label)
                ])
        for tfloat in "pt eta phi mass reclPhotonId mcMatchId".split():
            biglist.append( ("PhotonSel"+label+"_"+tfloat,"F",20,"nPhotonSel"+label) )
        biglist.append( ("PhotonSel"+label+"_pdgId","I",20,"nPhotonSel"+label) )
    ...
    def recleanPhotons(self, phocollcleaned, phocolldiscarded, lepcoll, postfix, ret, phoret, event):
        ### Define taus
        allphotons = phocollcleaned + phocolldiscarded
        # 0. mark each Photon as clean
        for t in allphotons: t._clean = True
        # 1. check for every photon if it is too close to a loose lepton
        for t in allphotons:
            for lep in lepcoll:
                dr = deltaR(lep, t)
                if self.cleanPhoton(lep, t, dr):
                    t._clean = False
        # 2. compute the Photon list
        ret["iPhotonSel"+postfix]=[]
        for itc, t in enumerate(phocollcleaned):
            if not t._clean        : continue
            if not self.loosePhoton(t): continue
            setattr(t, "reclPhotonId", 1 + self.tightPhoton(t))
            ret["iPhotonSel"+postfix].append(itc)

        # 3. sort the Photon by pt
        ret["iPhotonSel"+postfix].sort(key = lambda idx : phocollcleaned[idx].pt if idx >= 0 else phocolldiscarded[-1-idx].pt, reverse = True)
        goodphotons = [(phocollcleaned[idx] if idx >= 0 else phocolldiscarded[-1-idx]) for idx in ret["iPhotonSel"+postfix]]
        ret["nPhotonSel"      + postfix] = len(goodphotons)
        ret["nTightPhotonSel" + postfix] = sum([1 for g in goodphotons if g.reclPhotonId == 2])
        # 4. store the Photon 4-vectors
        if postfix==self.label:
            for tfloat in "pt eta phi mass reclPhotonId".split():
                phoret[tfloat] = []
                for g in goodphotons:
                    phoret[tfloat].append( getattr(g, tfloat) )
            for tfloat in "mcMatchId".split():
                phoret[tfloat] = []
                for g in goodphotons:
                    phoret[tfloat].append( getattr(g, tfloat) if hasattr(event,"PhoGood_"+tfloat) else -99 )
        return goodphotons
    ...
    def __call__(self, event):
        ...
        pho  = [p for p in Collection(event,"PhoGood","nPhoGood")]
        for l in leps:
        ...
        loosephotons = {}; phoret = {}
        retplabel = {}
        loosephotons = self.recleanPhotons(pho, [], lepsl if self.cleanPhotonsWithLoose else lepsc, self.label, retplabel, phoret, event)
        ...
        cleanjets={}
        for var in self.systsJEC:
            cleanjets[var] = self.recleanJets(jetsc[var],jetsd[var],lepsc + loosephotons if self.cleanJetsWithPhotons else lepsc,self.label+self.systsJEC[var],retwlabel,jetret,discjetret)
        #print "Jets def"
        ...
        fullret.update(retplabel)
        for k,v in tauret.iteritems(): 
            fullret["TauSel%s_%s" % (self.label,k)] = v
        for k,v in phoret.iteritems(): 
            fullret["PhotonSel%s_%s" % (self.label,k)] = v
