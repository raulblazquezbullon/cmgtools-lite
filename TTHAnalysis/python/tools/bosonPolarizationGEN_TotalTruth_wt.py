...
## bosonPolarizationWZGEN_wt
## ___________________________________________________________________
class bosonPolarizationGEN_TotalTruth_wt:
    ...
    def __call__(self, event):
        ...
        if not(self.lenZ == 2 and self.lenW == 1):
            if self.lenZ < 2: self.ret["status_wt"] = 1
            elif self.lenW < 1: self.ret["status_wt"] = 2
            elif self.lenZ > 2: self.ret["status_wt"] = 3 
            elif self.lenW > 1: self.ret["status_wt"] = 4
            #print "statused"
            return self.ret
        self.ret["status_wt"] = -1
        self.collectObjects(event)
        if self.ret["status_wt"] == 5: return self.ret
    ...
    def analyzeTopology(self):
        ...
        self.ret["genThetaWUp_HE_wt"] = (self.LorToWUp*self.pl3).Theta()
        self.ret["genThetaWDn_HE_wt"] = (self.LorToWDn*self.pl3).Theta()
        self.ret["genThetaZUp_HE_wt"] = (self.LorToZ*self.lepforZ).Theta()
        self.ret["genThetaZDn_HE_wt"] = (self.LorToZ*self.lepforZ).Theta() #Not a bug, there is just one solution in this case
        self.ret["cos_genThetaWDn_HE_wt"] = math.cos((self.LorToWDn*self.pl3).Theta())*self.charge
        self.ret["cos_genThetaZDn_HE_wt"] = math.cos((self.LorToZ*self.lepforZ).Theta())
        #Now get the other interesting angles
        self.ret["genThetaWUp_HE_CM_wt"] = (self.LorToWUp_CM*self.pl3).Theta()
        self.ret["genThetaWDn_HE_CM_wt"] = (self.LorToWDn_CM*self.pl3).Theta()
        self.ret["genThetaZUp_HE_CM_wt"] = (self.LorToZUp_CM*self.lepforZ).Theta()
        self.ret["genThetaZDn_HE_CM_wt"] = (self.LorToZDn_CM*self.lepforZ).Theta() #In this case they are different
        ...
        self.ret["genThetaW_CS_wt"] = self.pWUp.Pz()/abs(self.pWUp.Pz())/(self.pWUp.M()*(self.pWUp.M()**2 + self.pWUp.Pt()**2)**0.5)*((self.pl3.E()+self.pl3.Pz())*(self.met.E()-self.met.Pz()) - (self.pl3.E()-self.pl3.Pz())*(self.met.E()+self.met.Pz()))
        ...
        self.ret["genThetaZ_CS_wt"] = self.pZ.Pz()/abs(self.pZ.Pz())/(self.pZ.M()*(self.pZ.M()**2 + self.pZ.Pt()**2)**0.5)*((self.pl1.E()+self.pl1.Pz())*(self.pl2.E()-self.pl2.Pz()) - (self.pl1.E()-self.pl1.Pz())*(self.pl2.E()+self.pl2.Pz()))*self.islZ1
    ...
    def collectObjects(self, event):
        ...
        for part in ["Z", "WUp", "WDn", "WZUp", "WZDn"]:
            self.ret["gen" + part+"_pt_Lab_wt"] = getattr(self, "p" + part).Pt()
            self.ret["gen" +part+"_eta_Lab_wt"] = getattr(self, "p" + part).Eta()
            self.ret["gen" +part+"_phi_Lab_wt"] = getattr(self, "p" + part).Phi()
            self.ret["gen" +part+"_mass_Lab_wt"] = getattr(self, "p" + part).M()

        self.ret["charge_wt"] = self.charge
    def getNeuEta(self):
        #Solve for W mass
        phil = self.pl3.Phi()
        etal = self.pl3.Eta()
    ...
    def listBranches(self):
        ...
        biglist = [
            ...
            ("genThetaWUp_HE_CM_wt"            , "F"),
            ("genThetaWDn_HE_CM_wt"            , "F"),
            ("genThetaZUp_HE_CM_wt"            , "F"),
            ("genThetaZDn_HE_CM_wt"            , "F"),
            ("cos_genThetaWDn_HE_wt"           , "F"),
            ("cos_genThetaZDn_HE_wt"           , "F"),
            ("charge_wt", "I"),
            ("status_wt", "I"),
            ("cos_genThetaWDn_CS_wt", "F"),
            ("mW_genrec_wt", "F")]

        for part in ["Z", "WUp", "WDn", "WZUp", "WZDn"]:
            for var in ["pt", "eta", "phi", "mass"]:
                biglist.append(("gen" + part+"_" + var +"_Lab_wt", "F"))
        return biglist
