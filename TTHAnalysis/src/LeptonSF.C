...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>> Triggers
  ...
  cout << "> Lepton SF", Form("Loaded histogram %s from file %s%s.root", histoname.Data(), path_to_SF_histos.Data(), filename.Data()) << endl;
...
TH2* LeptonSF::GetHistogramFromFileD(const char* filename, const char* histoname, const char* newhname) {
  ...
  if (!file) {
    cout << "> LeptonSF", Form("Could not load file\"%s\"", filename) << endl;
    return 0;
  }
  TH2* h = (TH2*) file->Get(histoname)->Clone(newhname);
  if (!h) {
    cout << "> LeptonSF", Form("Could not find histogram \"%s\"", histoname) << endl;
    return 0;
  }
  h->SetDirectory(0);
  file->Close();
